...
// ============================================================================
// PoM v4 two-phase tensor-core solver (ported from keryx-miner-supr v0.11.5/0.11.6).
// The offset chain depends only on the nonce seed + tile snippets, never on matrix state,
// so a light chase kernel can resolve all K tile indices before the matrix walk. On sm_80+
// the walk then uses cp.async + INT8 mma.sync. The classic pom_mine_v4 above remains the
// fallback and consensus reference path.
// ============================================================================

__device__ __forceinline__ const ulonglong2* v4_tc_chunk_addr(
    const unsigned long long* bases, const unsigned long long* prefix, unsigned int T,
...
extern "C" __global__ void pom_mine_v4_chase(
    ...
    for (unsigned int step = 1; step <= K; step++) {
        ...
        const ulonglong2* q = v4_tc_chunk_addr(bases, prefix, T, off, 0);
        const ulonglong2 c0 = q[0], c1 = q[1];
        unsigned long long sf = 0;
        sf = mix64(sf ^ (unsigned int)(c0.x));
        sf = mix64(sf ^ (unsigned int)(c0.x >> 32));
        sf = mix64(sf ^ (unsigned int)(c0.y));
        sf = mix64(sf ^ (unsigned int)(c0.y >> 32));
        sf = mix64(sf ^ (unsigned int)(c1.x));
        sf = mix64(sf ^ (unsigned int)(c1.x >> 32));
        sf = mix64(sf ^ (unsigned int)(c1.y));
        sf = mix64(sf ^ (unsigned int)(c1.y >> 32));
        off = mix64(seed ^ (unsigned long long)(step + 1) * V4_OFFSET_STEP_SALT ^ sf) % n_tiles;
    }
}
...
__device__ __forceinline__ void v4_cp_async16(void* smem_dst, const void* gmem_src) {
    const unsigned long long sdst = __cvta_generic_to_shared(smem_dst);
...
__device__ __forceinline__ void v4_tile_cp_async(
    const unsigned long long* bases, const unsigned long long* prefix, unsigned int T,
    unsigned long long tile_index, unsigned int* s_tile, unsigned int lane) {
    const ulonglong2* q = v4_tc_chunk_addr(bases, prefix, T, tile_index, lane);
    ulonglong2* dst = (ulonglong2*)(s_tile + lane * 8);
    v4_cp_async16(dst, q);
...
extern "C" __global__ void pom_mine_v4_tc(
...

    {
        unsigned long long h = mix64(seed ^ (V4_S0_ROW_SALT + (unsigned long long)x));
        #pragma unroll
        for (int k4 = 0; k4 < V4_D4; k4++) {
            h = mix64(h);
            s_state[x * 8u + k4] = (unsigned int)h;
        }
    }
    __syncwarp();
    ...
    for (unsigned int step = 1; step <= K; step++) {
        ...
        if (step + V4_TC_PIPE - 2 < K) {
            v4_tile_cp_async(
                ...
                prefix,
                T,
                my[step + V4_TC_PIPE - 2],
                s_buf + ((step + V4_TC_PIPE - 2u) % V4_TC_PIPE) * 256u,
                x);
        }
        asm volatile("cp.async.commit_group;");
